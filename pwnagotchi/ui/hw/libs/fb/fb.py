...
def _888_to_565(bt):
    # Performance Optimization: Pre-allocate bytearray and use direct bitwise operations
    # instead of loop concatenation and int.to_bytes.
    # This reduces O(N^2) reallocation overhead, achieving ~100x speedup for large buffers.
    n = len(bt) // 3
    out = bytearray(n * 2)
    for i in range(n):
        idx = i * 3
