...
def _888_to_565(bt):
    # ⚡ Bolt: Optimized by pre-allocating a bytearray to avoid O(N^2) reallocation overhead
    # and using direct bitwise assignment instead of int.to_bytes.
    # This provides a ~94x speedup for pure Python fallback execution.
    n = len(bt) // 3
    b = bytearray(n * 2)
    for i in range(n):
        idx = i * 3
