...
def _888_to_565(bt):
    # Performance optimization: pre-allocate bytearray and use struct.pack_into
    # to avoid O(N^2) memory reallocation overhead when constructing large byte buffers.
    l = len(bt)
    out = bytearray((l // 3) * 2)
    j = 0
    for i in range(0, l, 3):
        struct.pack_into('<H', out, j, bt[i] >> 3 << 11 | bt[i + 1] >> 2 << 5 | bt[i + 2] >> 3)
        j += 2
    return bytes(out)
